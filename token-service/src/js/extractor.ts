...
import type { JsAnalyzer, VariableMetadata } from './analyzer.ts';
import { extractNodeSource, indent, jsBuiltIns, memberToString } from './helpers.ts';

export type SideEffectMode = 'strict' | 'loose';

export interface EmitterOptions {
  maxDepth?: number;
  disallowSideEffectInitializers?: boolean | { mode?: SideEffectMode };
...
export class JsExtractor {
  constructor(private analyzer: JsAnalyzer) {}
   ...
   */
  private isSafeInitializer(node?: ESTree.Node | null, mode: SideEffectMode = 'strict'): boolean {
    if (!node) return true;
   ...
   */
  private renderNode(node: ESTree.Node, preDeclared: boolean, options: EmitterOptions = {}): string {
   ...
   */
  private createWrapperFunction(name: string, node: ESTree.Node): string | undefined {
    if (
      ...
      // Handle Identifier nodes (e.g., from nMatcher which returns array element)
      // Look up the identifier to find if it's a function
      ...
      if (decl?.node?.type === 'VariableDeclarator' && decl.node.init?.type === 'FunctionExpression') {
        // The identifier points to a function, create wrapper
        ...
        // Identifier exists but may be a complex function, create simple wrapper
   ...
   */
  private generateWrapper(functionName: string, targetFunction: string, args: string): string {
    return [
   ...
   */
  private parseFunctionArguments(args: ESTree.Node[]): string {
    const params: string[] = [];
  ...
  public buildScript(config: EmitterOptions): BuildScriptResult {
    ...
    const registerPredeclaredVar = (name?: string) => {

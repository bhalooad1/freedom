...
import { getVideoPoToken, getVisitorData, isInitialized, refreshPoToken } from '../potoken/generator.ts';
import { getPlayerData, decipherUrl } from '../innertube/player.ts';
import { VideoUrlResponse, PlayerResponse, VideoFormat } from '../types.ts';
 ...
 */
function selectBestFormat(streamingData: PlayerResponse['streamingData']): VideoFormat | null {
...
app.post('/video-url', async (c) => {
  ...
  const body = (c.get('parsedBody') as any) || (await c.req.json());
  const videoId = body.videoId;

  if (!videoId || !/^[a-zA-Z0-9_-]{11}$/.test(videoId)) {
    return c.json<VideoUrlResponse>(
      {
        success: false,
